...
# lrs = [0.0005, 0.004, 0.004, 0.002, 0.001, 0.0007]
lrs = [0.005, 0.004, 0.004, 0.002, 0.001, 0.0007, 0.0005, 0.0002, 0.0001, 0.0001]
print(depths) #0.003, 0.0005, 0.0002, 0.002, 0.001,
MC = 5
_, dataloaders, criterion = get_task(args)
dir = args['dir']
task_dis = args['task'].split('_')[0] + '_' + str(args['width'])
...
def process(index, rank, lr, model, optimizer, loaders = dataloaders, args = args, task_dis = task_dis, depths = depths):
    ...
    for epoch in range(args['epochs']):
        ...
        if loss1 < 1e-1 and loss2 < 1e-1 and stop_2 == False:
        ...
        if loss1 < 2e-2 and loss2 < 2e-2:
            ...
            torch.save(model, os.path.join(result_dir, 'model_trained'))
            torch.save(model_prev, os.path.join(result_dir, 'model_prev'))
