...
args['depth'] = 7#6
args['width'] = 128 #256
args['task'] = 'fmnist_fcfree' #'mnist_fc'
...
lrs = [1e-5, 3e-5, 1e-4, 5e-4, 0.001, 0.003, 0.005, 0.01, 0.02, 0.05, 0.1]
...
def do_compute_ntk(iterations):
    ...
    # return iterations == 0 or iterations in (len(dataloaders['train'])) * (np.arange(args['epochs'])).astype('int') #iterations == 0 or iterations in 5 * (1.15 ** np.arange(300)).astype('int')
    ...
    for epoch in range(args['epochs']):
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloaders['train']):
            ...
            # if args.layer_align_test:
                ...
                # to_log['corr_W_y'] = similarity_w_0_y(W, dataloaders['micro_train'])
            ...
            # if args.align_test or args.save_ntk_test:
                ...
                log.to_pickle(os.path.join(result_dir,'log.pkl'))
