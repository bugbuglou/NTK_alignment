...
def get_esd_plot(eigenvalues, weights, name):
    ...
    plt.semilogy(grids, density + 1.0e-7, label = name.split('_')[-1])
    ...
    plt.tight_layout()
    plt.legend(loc='best', bbox_to_anchor=(0.8, 0.6, 0.5, 1))
    plt.savefig(name)
...
def compute_hessian(model, loader, num_eigenthing, cal_target = False):
    ...
    def Loss(inp, tar, W = W):
        return torch.sum(inp * W)/(W.shape[0]) + F.cross_entropy(inp, tar) - F.cross_entropy(inp, tar)
...
def compute_hessian_spectrum(model, loader, path, cal_target = False):
    ...
    def Loss_y(inp, tar, t = target):
        return torch.sum(inp * t)/(W.shape[0]) + F.cross_entropy(inp, tar) - F.cross_entropy(inp, tar)
    hessian_dataloader = []
...
lrs = [args.lr1, args.lr2, args.lr3] #, 0.0005, 0.0002, 0.001, 0.005, 0.01, 0.02, 0.05
# lrs = [1e-4, 5e-4]
...
def process(index, args):
    ...
    columns.append('density_eigen')
    columns.append('density_weight')
    ...
    for epoch in range(args['epochs']):
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloaders['train']):
            ...
            # # # if args.layer_align_test:
                ...
                to_log['eigenvals'], to_log['eigenvecs'] = compute_hessian(model, dataloaders['micro_train'],
                                                                  args['num_eigenthings'],cal_target = False)
                ...
                to_log['density_eigen'], to_log['density_weight'] = compute_hessian_spectrum(model, dataloaders['micro_train'], os.path.join(result_dirs[index],'specturm_' + str(epoch)), cal_target = False)
                to_log['epoch'] = epoch
                ...
                log.to_pickle(os.path.join(result_dirs[index],'log2_eigs_changed.pkl'))
