...
parser.add_argument('--task', required=True, type=str, help='Task',
                    choices=['mnist_fcfree', 'fmnist_fcfree', 'cifar10_fcfree', 'cifar100_fcfree', 'cifar10_vgg19','cifar10_vgg11','cifar10_vgg13', 'cifar10_vgg16', 'cifar10_resnet18', 'cifar100_vgg19', 'cifar100_resnet18'， 'kmnist_fcfree'])
...
def ResNet18(num_classes = 10, bn=True):
    return ResNet(BasicBlock, [2,2,2,2], num_classes= num_classes,bn=bn)
...
model1, dataloaders, criterion = get_task(args)
dir = args['dir']
task_dis = args['task'] + '_' + str(args['depth']) + '_' + str(args['width'])
...
for i in range(len(lrs)):
    ...
    # model_alt = FashionCNN(args['fmncnn'])
    # model_alt = ResNet18()
...
# Training
def train(model, optimizer, scheduler, args, log, result_dir):
    def output_fn(x, t):
        return model(x)
    ...
    for epoch in range(args['epochs']):
        ...
        #     torch.save(model, os.path.join(result_dir,'model.pkl'))
        #     print('stopping now')
        #     break
...
#                                     centering=not args['no_centering'])
            ...
            # # if args.align_test or args.save_ntk_test:
                ...
                log.to_pickle(os.path.join(result_dir,'log_correlation.pkl'))
...
# # columns.append('W')
# columns.append('w_squared')
# columns.append('w_squared_test')
columns.append('corr_y_train')
columns.append('corr_y_test')
# columns.append('two_terms_train')
# columns.append('two_terms_test')
...
#     print(i)
    ...
    train(models[i], optimizers[i], schedulers[i], args, log, result_dirs[i])
